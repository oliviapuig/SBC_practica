...
from clustering_llibres import cluster_dataframe, cluster_book, get_cluster_books
...
class CBR:
    ...
    def revise(self, user, llibres_reomanats):
        """
        Calculem la similitud de l'usuari respecte els llibres pertanyents al cluster dels llibres recomanats
        """
        similarities = []
        for llibre in llibres_reomanats:
            cluster = cluster_book(llibre)
            cluster_books = get_cluster_books(cluster)
            if ll in cluster_books:
                similarities.append(self.similarity(llibre, ll, 'cosine'))
        return user
